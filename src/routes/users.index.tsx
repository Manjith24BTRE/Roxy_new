...
import { fmtDate, relative, type User, useControlCenterData } from "@/lib/control-center-data";
...
function UsersPage() {
  const { users } = useControlCenterData();
  ...
  const rows = useMemo(
    () =>
      users.filter((u) => {
        ...
        return (
          match && (status === "all" || u.status === status) && (plan === "all" || u.plan === plan)
        );
      }),
    [search, status, plan],
  );
  ...
  const columns: Column<User>[] = [
    ...
    },
    {
      key: "plan",
      header: "Plan",
      render: (u) => <span className="text-sm">{u.plan}</span>,
      hideBelow: "md",
    },
    {
      key: "credits",
      header: "Credits",
      render: (u) => <span className="num">{u.credits.toLocaleString()}</span>,
      hideBelow: "xl",
    },
    {
      key: "usage",
    ...
    { key: "status", header: "Status", render: (u) => <StatusBadge status={u.status} /> },
    {
      key: "last",
      header: "Last login",
      render: (u) => <span className="num text-xs">{relative(u.lastLogin)}</span>,
      hideBelow: "md",
    },
    {
      key: "created",
      header: "Created",
      render: (u) => <span className="num text-xs">{fmtDate(u.createdAt)}</span>,
      hideBelow: "xl",
    },
    {
      key: "actions",
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <StatCard label="Total accounts" value={users.length.toLocaleString()} icon={UsersIcon} />
        <StatCard label="Active" value={String(active)} tone="success" />
        <StatCard
          label="Suspended"
        ...
        <StatCard
          ...
          value={
            users.length
              ? `${Math.round((users.filter((u) => u.twoFactor).length / users.length) * 100)}%`
              : "No data available"
          }
        />
      </div>
      ...
      <ConfirmationDialog
        ...
        onConfirm={() => {
          toast.success("Action applied", {
            description: `${pending?.action} · ${pending?.user.email}`,
          });
          setPending(null);
        }}
      />
