  ...
  head: () => ({
    meta: [
      { title: "Credits — Veytrix Control Centre" },
      {
        name: "description",
        content:
          "Credit ledger covering purchases, consumption, refunds and administrative grants.",
      },
...
function CreditsPage() {
  const { creditTransactions, creditsSeries } = useControlCenterData();
  ...
  const rows = useMemo(
    () =>
      creditTransactions.filter(
        (c) =>
          (!search ||
            [c.user, c.reference, c.id].some((f) =>
  ...
  const columns: Column<CreditTx>[] = [
    {
      ...
      header: "Entry",
      render: (c) => (
        <CellStack primary={<Mono className="text-foreground">{c.id}</Mono>} secondary={c.user} />
      ),
    },
    {
      key: "type",
      ...
      render: (c) => (
        <StatusBadge
          status={c.type === "consumption" ? "info" : c.type === "refund" ? "warning" : "success"}
          label={c.type}
        />
      ),
    },
    ...
    },
    {
      ...
      render: (c) => <span className="num">{c.balance.toLocaleString()}</span>,
      hideBelow: "md",
    },
    { key: "ref", header: "Reference", render: (c) => <Mono>{c.reference}</Mono>, hideBelow: "lg" },
    {
      key: "date",
      header: "Date",
      render: (c) => <span className="num text-xs">{fmtDateTime(c.date).slice(0, 16)}</span>,
      hideBelow: "md",
    },
  ];
    ...
    <>
      <PageHeader
        ...
        actions={
          <Button
            size="sm"
            onClick={() =>
              toast.success("Credits granted", { description: "5,000 credits added." })
            }
          >
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        ...
        <StatCard
          label="Refunded"
          value={compact(
            creditTransactions.filter((c) => c.type === "refund").reduce((a, c) => a + c.amount, 0),
          )}
          delta={-18.9}
          invertDelta
          tone="success"
            ...
            filters={[
              {
                ...
                value: type,
                onChange: setType,
                options: ["purchase", "consumption", "refund", "grant"].map((t) => ({
                  label: t,
                  value: t,
                })),
              },
