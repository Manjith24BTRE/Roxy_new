...
import { fmtDate, money, type Transaction, useControlCenterData } from "@/lib/control-center-data";
...
function TransactionsPage() {
  const { transactions } = useControlCenterData();
  ...
  const rows = useMemo(
    () =>
      transactions.filter((t) => {
        ...
        const match =
          !q || [t.id, t.user, t.invoice, t.method].some((f) => f.toLowerCase().includes(q));
        return (
          match && (status === "all" || t.status === status) && (plan === "all" || t.plan === plan)
        );
      }),
    [search, status, plan],
  );
  ...
  const columns: Column<Transaction>[] = [
    {
      ...
      render: (t) => (
        <CellStack
          primary={<Mono className="text-foreground">{t.id}</Mono>}
          secondary={t.invoice}
        />
      ),
    },
    {
      key: "user",
      header: "User",
      render: (t) => <span className="text-sm">{t.user}</span>,
      hideBelow: "md",
    },
    {
      key: "plan",
      header: "Plan",
      render: (t) => <span className="text-sm">{t.plan}</span>,
      hideBelow: "lg",
    },
    {
      key: "amount",
      header: "Amount",
      render: (t) => <span className="num font-medium">{money(t.amount)}</span>,
    },
    {
      key: "method",
      header: "Method",
      render: (t) => <span className="text-xs text-muted-foreground">{t.method}</span>,
      hideBelow: "xl",
    },
    { key: "status", header: "Status", render: (t) => <StatusBadge status={t.status} /> },
    {
      key: "date",
      header: "Date",
      render: (t) => <span className="num text-xs">{fmtDate(t.date)}</span>,
      hideBelow: "md",
    },
    {
      key: "actions",
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <StatCard label="Gross volume" value={money(gross)} icon={Receipt} tone="success" />
        <StatCard label="Refunded" value={money(refunded)} />
        <StatCard label="Failed payments" value={String(failed)} tone="danger" />
        <StatCard
          label="Avg order value"
          value={transactions.length ? money(gross / transactions.length) : "No data available"}
        />
      </div>
      <DataTable
        columns={columns}
            ...
            filters={[
              {
                ...
                value: status,
                onChange: setStatus,
                options: ["success", "pending", "failed", "refunded"].map((s) => ({
                  label: s,
                  value: s,
                })),
              },
              {
                ...
                value: plan,
                onChange: setPlan,
                options: ["Starter", "Growth", "Scale", "Enterprise"].map((s) => ({
                  label: s,
                  value: s,
                })),
              },
