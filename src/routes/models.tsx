...
import { compact, relative, useControlCenterData } from "@/lib/control-center-data";
  ...
  head: () => ({
    meta: [
      { title: "AI Models — Veytrix Control Centre" },
      {
        name: "description",
        content:
          "Model catalogue, versions, throughput, error rates and failure monitoring for Veytrix inference.",
      },
...
type Model = ReturnType<typeof useControlCenterData>["models"][number];

function ModelsPage() {
  const { jobs, models, modelMixSeries } = useControlCenterData();
  const columns: Column<Model>[] = [
    {
      ...
      render: (m) => (
        <CellStack
          primary={<Mono className="text-foreground">{m.name}</Mono>}
          secondary={m.family}
        />
      ),
    },
    {
      ...
      render: (m) => <Mono>{m.versions[0]}</Mono>,
      hideBelow: "md",
    },
    { key: "status", header: "Status", render: (m) => <StatusBadge status={m.status} /> },
    {
      key: "latency",
      header: "p95 latency",
      render: (m) => <span className="num">{m.latencyMs} ms</span>,
      hideBelow: "lg",
    },
    {
      key: "jobs",
      header: "Jobs (24h)",
      render: (m) => <span className="num">{m.jobs24h.toLocaleString()}</span>,
      hideBelow: "md",
    },
    {
      key: "err",
      header: "Error rate",
      render: (m) => (
        <span className={m.errorRate > 2 ? "num text-destructive" : "num"}>{m.errorRate}%</span>
      ),
    },
    {
      key: "cost",
      header: "Cost / 1k",
      render: (m) => <span className="num">${m.costPer1k}</span>,
      hideBelow: "xl",
    },
  ];
  ...
  return (
    <>
          ...
          <Button
            ...
            onClick={() =>
              toast.success("Deployment queued", {
                description: "veytrix-text-4.2.2 rolling out at 10%.",
              })
            }
          >
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        ...
        <StatCard
          label="Versions live"
          value={String(models.reduce((a, m) => a + m.versions.length, 0))}
          icon={GitBranch}
        />
        <StatCard
          label="Jobs (24h)"
          value={compact(models.reduce((a, m) => a + m.jobs24h, 0))}
          delta={9.4}
        />
        <StatCard
      ...
      <Tabs defaultValue="catalogue" className="space-y-3">
        ...
        <TabsContent value="versions" className="m-0 grid grid-cols-1 gap-3 lg:grid-cols-2">
          {models.map((m) => (
            <Panel
              key={m.id}
              title={m.name}
              description={`${m.family} · ${m.versions.length} versions`}
            >
        ...
        <TabsContent value="analytics" className="m-0 grid grid-cols-1 gap-3 xl:grid-cols-3">
          ...
          <ChartCard
            ...
            description="p95 in milliseconds"
