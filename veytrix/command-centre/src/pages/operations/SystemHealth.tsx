...
import { RefreshCw, Server, Activity, Database, Key, HardDrive, Mail, CreditCard } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#64748B] mt-1">Real-time status of all core infrastructure and third-party services.</p>
        </div>
        <button 
          onClick={handleRefresh}
          className="flex items-center gap-2 bg-white border border-[#E2E8F0] px-4 py-2 rounded-lg text-sm font-semibold text-[#1D2B64] hover:bg-[#F8FAFC] transition-colors shadow-sm w-fit"
        >
          ...
          return (
            <div key={sys.service} className="bg-white p-5 rounded-2xl border border-[#E2E8F0] shadow-sm flex flex-col group hover:border-[#3B6CE7]/30 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-4 mt-4 pt-4 border-t border-[#F1F5F9]">
                <div>
                  <p className="text-[10px] font-bold text-[#64748B] uppercase tracking-wider mb-1">Latency</p>
